...
import { Post, BookPost, MoviePost, TvShowPost, RestaurantPost, PlacePost } from "../models/Post.js";
...
export const createPost = asyncHandler(async (req, res) => {
  const { title, description, category, author, genre, director, network, address, cuisine, location, image } = req.body;
  ...
  switch (category) {
    case 'book':
      post = await BookPost.create({ title, description, image, author, genre, comments: [], user: req.user._id });
      break;
    case 'movie':
      post = await MoviePost.create({ title, description, image, director, genre, comments: [], user: req.user._id });
      break;
    case 'tv_show':
      post = await TvShowPost.create({ title, description, image, network, genre, comments: [], user: req.user._id });
      break;
    case 'restaurant':
      post = await RestaurantPost.create({ title, description, image, address, cuisine, comments: [], user: req.user._id });
      break;
    case 'place':
      post = await PlacePost.create({ title, description, image, location, comments: [], user: req.user._id });
      break;
    default:
      res.status(400);
...
export const updatePost = asyncHandler(async (req, res) => {
  const { title, description, author, genre, director, network, seasons, address, cuisine, location, image } = req.body;
  const post = await Post.findById(req.params.id);
